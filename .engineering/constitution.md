...
`@maw/*` and reused by every product (Restaurant OS, Sushmapet, and future apps), across
...
1. **Contracts over implementations.** Logic depends on a port (an interface in
   `@maw/sdk/contracts`), never on a concrete engine, framework, or database. This is
...
3. **One resolver for authorization.** `resolveEffectiveAccess()` in `@maw/rbac-core` is
   the single `can()` used by both client nav-gates and server enforcement. Never
   duplicate the decision.
4. **Money is integer minor units** via `@maw/sdk/kernel/money`. Never float/decimal.
5. **No `any`.** No hardcoded secrets — URLs/keys come from env; strings from i18n;
   colors/spacing from `@maw/theme`.
6. **Multi-tenant by default.** Every access decision is scoped by `tenantId`.
7. **Reuse before create; configure, don't fork.** Products consume `@maw/*` as
